...
/// <summary>
/// Screen for viewing publishing sites report task details and results.
/// </summary>
public class PublishingSitesDetailScreen : BaseScreen
{
    ...
    private PublishingSitesReportResult? _currentResult;
    ...
    public override string ScreenTitle => _task?.Name ?? "Publishing Sites Report";
    ...
    private void InitializeUI()
    {
        ...
        // Sites tab
        var sitesTab = new TabPage("Publishing Sites");

        var sitesHeaderPanel = new FlowLayoutPanel
        {
            Dock = DockStyle.Top,
            Height = 35,
            FlowDirection = FlowDirection.LeftToRight,
        ...
        _filterStatusCombo.Items.Add("Publishing Only");
        _filterStatusCombo.Items.Add("All Sites");
        _filterStatusCombo.Items.Add("Both Active");
        _filterStatusCombo.Items.Add("Infrastructure Only");
        _filterStatusCombo.Items.Add("Web Only");
        _filterStatusCombo.Items.Add("Not Active");
        ...
        {
            Width = 200,
            PlaceholderText = "Site URL or title..."
        };
        _searchTextBox.TextChanged += SearchTextBox_TextChanged;

        sitesHeaderPanel.Controls.AddRange(new Control[]
        {
        ...
        {
            if (_sitesGrid.CurrentCell != null)
            {
                Clipboard.SetText(_sitesGrid.CurrentCell.Value?.ToString() ?? "");
            }
        });
        copyMenuItem.ShortcutKeys = Keys.Control | Keys.C;
        var copyRowMenuItem = new ToolStripMenuItem("Copy Row", null, (s, e) =>
        {
            if (_sitesGrid.CurrentRow != null)
            {
                var values = new List<string>();
                foreach (DataGridViewCell cell in _sitesGrid.CurrentRow.Cells)
                {
        ...
        _sitesGrid.ContextMenuStrip = gridContextMenu;

        _sitesGrid.Columns.Add("SiteUrl", "Site URL");
        _sitesGrid.Columns.Add("SiteTitle", "Site Title");
        _sitesGrid.Columns.Add("Infrastructure", "Publishing Infrastructure");
        _sitesGrid.Columns.Add("WebPublishing", "Publishing Web");
        _sitesGrid.Columns.Add("PublishingStatus", "Publishing Status");
        _sitesGrid.Columns.Add("Status", "Status");

        _sitesGrid.Columns["SiteUrl"]!.FillWeight = 160;
        _sitesGrid.Columns["SiteTitle"]!.FillWeight = 120;
        _sitesGrid.Columns["Infrastructure"]!.FillWeight = 80;
        _sitesGrid.Columns["WebPublishing"]!.FillWeight = 80;
        _sitesGrid.Columns["PublishingStatus"]!.FillWeight = 80;
        _sitesGrid.Columns["Status"]!.FillWeight = 60;

        sitesTab.Controls.Add(_sitesGrid);
        sitesTab.Controls.Add(sitesHeaderPanel);
        ...
        _summaryList.Columns.Add("Metric", 250);
        _summaryList.Columns.Add("Value", 120);
        ...
        _tabControl.TabPages.AddRange(new[] { sitesTab, summaryTab, logTab });
    ...
    private async Task RefreshTaskDetailsAsync()
    {
        ...
        _currentResult = await _taskService.GetLatestPublishingSitesReportResultAsync(_task.Id);
    ...
    private void DisplayResults(PublishingSitesReportResult result)
    {
        DisplaySites(result);
    ...
    private void DisplaySites(PublishingSitesReportResult result)
    {
        _sitesGrid.Rows.Clear();

        var sites = result.SiteResults.ToList();

        // Apply status filter
        var statusFilter = _filterStatusCombo.SelectedItem?.ToString();
        if (statusFilter == "Publishing Only")
        {
            sites = sites.Where(s => s.Success && s.HasPublishing).ToList();
        }
        else if (statusFilter == "Both Active")
        {
            sites = sites.Where(s => s.Success && s.HasPublishingInfrastructure && s.HasPublishingWeb).ToList();
        }
        else if (statusFilter == "Infrastructure Only")
        {
            sites = sites.Where(s => s.Success && s.HasPublishingInfrastructure && !s.HasPublishingWeb).ToList();
        }
        else if (statusFilter == "Web Only")
        {
            sites = sites.Where(s => s.Success && !s.HasPublishingInfrastructure && s.HasPublishingWeb).ToList();
        }
        ...
        if (!string.IsNullOrEmpty(searchText))
        {
            sites = sites.Where(s =>
                ...
                s.SiteTitle.Contains(searchText, StringComparison.OrdinalIgnoreCase)).ToList();
        }
        ...
        foreach (var site in sites)
        {
            var rowIndex = _sitesGrid.Rows.Add(
                site.SiteUrl,
                site.SiteTitle,
                site.HasPublishingInfrastructure ? "Yes" : "No",
                site.HasPublishingWeb ? "Yes" : "No",
                site.PublishingStatus,
            ...
            if (!site.Success)
            {
                _sitesGrid.Rows[rowIndex].DefaultCellStyle.BackColor = Color.FromArgb(255, 200, 200); // red
            }
            else if (site.HasPublishingInfrastructure && site.HasPublishingWeb)
            {
                _sitesGrid.Rows[rowIndex].DefaultCellStyle.BackColor = Color.FromArgb(255, 220, 180); // orange - both active
            }
            else if (site.HasPublishing)
            {
                _sitesGrid.Rows[rowIndex].DefaultCellStyle.BackColor = Color.FromArgb(255, 255, 200); // yellow - partial
            }
            // No color for non-publishing sites
        }

        SetStatus($"Showing {sites.Count} sites | Publishing: {result.PublishingSitesCount} ({result.BothActiveCount} both, {result.InfraOnlyCount} infra only, {result.WebOnlyCount} web only) | Not Active: {result.NonPublishingSitesCount}");
    }

    private void DisplaySummary(PublishingSitesReportResult result)
    {
        _summaryList.Items.Clear();

        AddSummaryItem("Total Sites Processed", result.TotalSitesProcessed.ToString());
        AddSummaryItem("Successful Sites", result.SuccessfulSites.ToString());
        AddSummaryItem("Failed Sites", result.FailedSites.ToString());
        AddSummaryItem("", "");
        AddSummaryItem("Sites with Publishing Features", result.PublishingSitesCount.ToString());
        AddSummaryItem("  Both Active (Infra + Web)", result.BothActiveCount.ToString());
        AddSummaryItem("  Infrastructure Only", result.InfraOnlyCount.ToString());
        AddSummaryItem("  Web Only", result.WebOnlyCount.ToString());
        AddSummaryItem("Sites without Publishing", result.NonPublishingSitesCount.ToString());
        AddSummaryItem("", "");
        AddSummaryItem("Duration", result.Duration.ToString(@"hh\:mm\:ss"));
    }

    private void AddSummaryItem(string metric, string value)
    {
    ...
    private async Task ExecuteTaskAsync()
    {
        ...
        try
        {
            _currentResult = await _taskService.ExecutePublishingSitesReportAsync(
                _task,
                _authService,
                progress,
            ...
            {
                SetStatus($"Task completed. {_currentResult.PublishingSitesCount} publishing sites found across {_currentResult.SuccessfulSites} sites.");
            }
            else
            {
    ...
    private void ExportButton_Click(object? sender, EventArgs e)
    {
        ...
        var publishingOnly = _filterStatusCombo.SelectedItem?.ToString() == "Publishing Only";

        var safeName = SanitizeFileName(_task.Name);
        var suffix = publishingOnly ? "Publishing" : "AllSites";
        ...
        if (dialog.ShowDialog() == DialogResult.OK)
        {
            try
            {
                _csvExporter.ExportPublishingSitesReport(_currentResult, dialog.FileName, publishingOnly);
    ...
    private void ExportSummaryButton_Click(object? sender, EventArgs e)
    {
        ...
        if (dialog.ShowDialog() == DialogResult.OK)
        {
            try
            {
                _csvExporter.ExportPublishingSitesReportSummary(_currentResult, dialog.FileName);
                SetStatus($"Exported to {dialog.FileName}");
                OfferToOpenFile(dialog.FileName);
            }
            catch (Exception ex)
