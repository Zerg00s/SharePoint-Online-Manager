...
public class NavigationSettingsConfigScreen : BaseScreen
{
    ...
    private async Task LoadConnectionsAsync()
    {
        ...
        try
        {
            ...
            {
                // If we have a context, pre-select the source connection
                if (_context != null)
                {
                    var sourceIndex = _connections.FindIndex(c => c.Id == _context.Connection.Id);
                    if (sourceIndex >= 0)
                    {
                        _sourceConnectionCombo.SelectedIndex = sourceIndex;
                        // Select a different connection as target if available
                        if (_connections.Count > 1)
                        {
                            _targetConnectionCombo.SelectedIndex = sourceIndex == 0 ? 1 : 0;
                        }
                        else
                        {
