...
/// <summary>
/// Screen for executing and viewing Add Site Collection Administrators task results.
/// </summary>
public class AddSiteAdminsDetailScreen : BaseScreen
{
    ...
    public override string ScreenTitle => _task?.Name ?? "Add Site Admins";
    ...
    private void InitializeUI()
    {
        ...
        _resultsGrid.Columns.Add("Admin", "Admin Added");
    ...
    private void UpdateUI()
    {
        ...
        if (_config?.Administrators.Count > 0)
        {
            ...
            _adminsLabel.Text = $"Adding: {adminNames}";
        }
        else
        {
    ...
    private async void RunButton_Click(object? sender, EventArgs e)
    {
        ...
        try
        {
            ...
            for (int siteIndex = 0; siteIndex < _task.TargetSiteUrls.Count; siteIndex++)
            {
                ...
                var addedAdmins = new List<string>();
                ...
                foreach (var admin in _config?.Administrators ?? [])
                {
                    ...
                    try
                    {
                        var result = await spService.AddSiteCollectionAdminAsync(siteUrl, admin.LoginName);

                        if (result.IsSuccess)
                        {
                            addedAdmins.Add(admin.DisplayName);
                        }
                        else
                        {
                            siteSuccess = false;
                ...
                // Update grid row
                _resultsGrid.Rows[siteIndex].Cells["Admin"].Value = string.Join(", ", addedAdmins);

                if (siteSuccess && addedAdmins.Count > 0)
                {
